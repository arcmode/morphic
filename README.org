...
1) Define modularity model via a /platform/ function described by the pseudocode: ~platform = function (modules) -> features~
...
In any modular software, any work required to /migrate/ a given module into a different /framework/ should be considered evitable and the
patterns that have contributed should be considered anti-patterns. Please note that the previous claim is explicit about changes to module code,
