...
Before worrying about boilerplate code yet, let's define our /Rest module/ model in TypeScript trying to satisfy the /minimum viable interface/
that would be required to define what the module /does/ in the context of the specific project (Rest API).
